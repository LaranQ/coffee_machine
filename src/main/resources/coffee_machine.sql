...
(
    ...
    CONSTRAINT coffee_machine_check_high_border CHECK (water < 10000 AND milk < 3000 AND beans < 2000),
    CONSTRAINT coffee_machine_check_lower_border CHECK (water >= 0 AND milk >= 0 AND beans >= 0),
    CONSTRAINT coffee_machine_count_before_dirty_check CHECK (count_before_dirty >= 0 AND count_before_dirty < 6)
);
